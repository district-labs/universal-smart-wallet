...
/// @title Universal Smart Wallet Factory
/// @notice UniversalSmartWallet factory, based on Solady's ERC4337Factory.
/// @author District Labs (https://github.com/district-labs/universal-smart-wallet)
...
contract UniversalWalletFactory {
    ...
    /// @notice Thrown when trying to create a new `UniversalWallet` account without any owner.
    error OwnerRequired();

    /// @notice Factory constructor used to initialize the implementation address to use for future
    ///         UniversalWallet deployments.
    ///
    /// @param implementation_ The address of the UniversalWallet implementation which new accounts will proxy to.
    constructor(address implementation_) payable {
        implementation = implementation_;
    }

    /// @notice Returns the deterministic address for a UniversalWallet created with `owners` and `nonce`
    ...
    function createAccount(
        ...
        payable
        virtual
        returns (UniversalWallet account)
    {
        if (owners.length == 0) {
        ...
        account = UniversalWallet(payable(accountAddress));
    ...
    function _getSalt(bytes[] memory owners, uint256 nonce) internal pure returns (bytes32) {
